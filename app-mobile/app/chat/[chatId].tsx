  ...
  useEffect(() => {
    const loadChatUserInfo = async () => {
      ...
      try {
        ...
        const conversation = response.metadata.find(
          (conv: Conversation) => conv.sender.id === conversationUserId
        );
        if (conversation) {
          setChatUserName(conversation.sender.username || 'Người dùng');
        }
      } catch (error) {
        console.error('Failed to load chat user info:', error);
      }
    };
    loadChatUserInfo();
  }, [conversationUserId]);
