import { ScrollView, View, Text, StyleSheet, TouchableOpacity, FlatList, Alert, Modal, Image, Platform } from 'react-native';
  ...
  return (
    <View style={styles.container}>
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        {/* Categories - Modern minimalist design */}
        <View style={styles.section}>
          ...
          <FlatList
            data={[
              ...
              { id: 'o-ghep', name: 'Ở ghép', value: 'o-ghep' },
            ]}
            renderItem={({ item }) => (
              <TouchableOpacity
                style={[
                  styles.modernCategoryItem,
                  activeFilters.category === item.value && styles.modernCategoryItemSelected
                ]}
                onPress={() => handleCategoryPress(item.id)}
              >
                <View style={[
                  styles.modernCategoryIconContainer,
                  activeFilters.category === item.value && styles.modernCategoryIconContainerSelected
                ]}>
                  <Image
                    source={
                      item.id === 'phong-tro' ? require('../../assets/phongtro.png') :
                      item.id === 'nha-nguyen-can' ? require('../../assets/nharieng.png') :
                      item.id === 'can-ho-chung-cu' ? require('../../assets/Canhochungcu.png') :
                      item.id === 'can-ho-mini' ? require('../../assets/Canhochungcu.png') :
                      item.id === 'o-ghep' ? require('../../assets/oghep.png') :
                      require('../../assets/placeholder.png')
                    }
                    style={[
                      styles.modernCategoryIcon,
                      activeFilters.category === item.value && styles.modernCategoryIconSelected
                    ]}
                    resizeMode="contain"
                  />
                </View>
                <Text style={[
                  ...
                  activeFilters.category === item.value && styles.modernCategoryTextSelected
                ]}>{item.name}</Text>
              </TouchableOpacity>
            )}
        ...
        {/* Price Range Filter - Modern minimalist design */}
        <View style={styles.section}>
          <Text style={styles.sectionTitle}>Xem theo khoảng giá</Text>
          <FlatList
            data={[
              { range: 'duoi-1-trieu', label: 'Dưới 1 triệu' },
              { range: 'tu-1-2-trieu', label: '1-2 triệu' },
              { range: 'tu-2-3-trieu', label: '2-3 triệu' },
              { range: 'tu-3-5-trieu', label: '3-5 triệu' },
              { range: 'tu-5-7-trieu', label: '5-7 triệu' },
              { range: 'tu-7-10-trieu', label: '7-10 triệu' },
              { range: 'tu-10-15-trieu', label: '10-15 triệu' },
              { range: 'tren-15-trieu', label: 'Trên 15 triệu' },
            ]}
            renderItem={({ item }) => (
              <TouchableOpacity
                style={[
                  styles.modernFilterItem,
                  isPriceRangeActive(item.range) && styles.modernFilterItemSelected
                ]}
                onPress={() => handlePriceRangePress(item.range)}
              >
                <Text style={[
                  styles.modernFilterText,
                  isPriceRangeActive(item.range) && styles.modernFilterTextSelected
                ]}>{item.label}</Text>
              </TouchableOpacity>
            )}
        ...
        {/* Area Filter - Modern minimalist design */}
        <View style={styles.section}>
          <Text style={styles.sectionTitle}>Xem theo diện tích</Text>
          <FlatList
            data={[
              { range: 'duoi-20', label: '<20m²' },
              { range: 'tu-20-30', label: '20-30m²' },
              { range: 'tu-30-50', label: '30-50m²' },
              { range: 'tu-50-70', label: '50-70m²' },
              { range: 'tu-70-90', label: '70-90m²' },
              { range: 'tren-90', label: '>90m²' },
            ]}
            renderItem={({ item }) => (
              <TouchableOpacity
                style={[
                  styles.modernFilterItem,
                  isAreaRangeActive(item.range) && styles.modernFilterItemSelected
                ]}
                onPress={() => handleAreaRangePress(item.range)}
              >
                <Text style={[
                  styles.modernFilterText,
                  isAreaRangeActive(item.range) && styles.modernFilterTextSelected
                ]}>{item.label}</Text>
              </TouchableOpacity>
            )}
            keyExtractor={(item) => item.range}
...
const styles = StyleSheet.create({
  ...
  modernCategoryIconContainerSelected: {
  ...
  modernCategoryItemSelected: {
    // Only highlight the text and icon, not the container
  },
